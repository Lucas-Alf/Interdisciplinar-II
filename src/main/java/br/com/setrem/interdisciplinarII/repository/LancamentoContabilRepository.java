...
public interface LancamentoContabilRepository extends JpaRepository<LancamentoContabil, Integer> {
    ...
    public void insert(double valor, int centrocustoid, String historico, String clifoid, String tipo, int idconta,
            Date datahora);

    @Modifying
    @Transactional
    @Query(value = "delete from lancamentocontabil where historico LIKE ?1%", nativeQuery = true)
    public void DeletarLancamentoCont(String par);
}